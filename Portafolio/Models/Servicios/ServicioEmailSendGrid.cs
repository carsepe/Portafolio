...
namespace Portafolio.Models.Servicios
{
    ...
    public class ServicioEmailSendGrid : IServicioEmail
    {
        ...
        public async Task Enviar(ContactoViewModel contacto)
        {
            ...
            var contenidoHmtl = $@"De: {contacto.Nombre} - Email: {contacto.Email} Mensaje {contacto.Mensaje} ";
            var singleEmail = MailHelper.CreateSingleEmail(from, to, subject, mensajeTextoPlano, contenidoHmtl);
